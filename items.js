    ...
    function ItemDAO(database) {
      ...
      this.getCategories = function(callback) {
          ...
          this.db.collection("item").aggregate(
            ...
            { "$sort": { "_id": 1 } }
              ...
              for (var i = 0; i < documents.length; i++) {
                total += documents[i]['num'];
                categories.push(documents[i]);
              }
              category.num = total;
        ...
        this.searchItems = function(query, page, itemsPerPage, callback) {
          ...
          cursor = this.db.collection("item").find(queryDoc);
          cursor.limit(itemsPerPage);
          cursor.skip(itemsPerPage * page);
          cursor.sort({"_id": 1});
          cursor.toArray(function(err, docs){
        ...
        this.getItem = function(itemId, callback) {
           ...
           */
           var item = null;
           let cursor = this.db.collection("item").find({"_id": itemId});
        ...
        this.addReview = function(itemId, comment, name, stars, callback) {
          ...
          let cursor = this.db.collection("item").updateOne(
            {"_id": itemId}, {"$push": {"reviews": reviewDoc}},
            function(err, doc) {
              assert.equal(err, null);
              callback(doc);
            });
